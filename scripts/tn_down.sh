...
# specific BetterChain daemon processes.
#


if [ "$PWD" != "$BETTERCHAIN_HOME" ]; then
    echo $0 must only be run from $BETTERCHAIN_HOME
    exit -1
fi
...
if [ -z "$BETTERCHAIN_TN_RESTART_DATA_DIR" ]; then
    echo data directory not set
    exit 1
fi

DD=$BETTERCHAIN_TN_RESTART_DATA_DIR;
runtest=""
if [ $DD = "all" ]; then
    runtest=$prog
else
