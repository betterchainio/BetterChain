...
#include <betterchain/chain/fork_database.hpp>
#include <betterchain/chain/block_log.hpp>
...
#include <betterchain/chain/wast_to_wasm.hpp>
...
using namespace betterchain::chain::config;
using fc::flat_map;
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   if (options.at("skip-transaction-signatures").as<bool>()) {
      ...
      elog("Setting skip_transaction_signatures\n"
           ...
           "**************************************\n"
           "*                                    *\n"
           "*   -- BetterChainD IGNORING SIGNATURES --   *\n"
           "*   -         TEST MODE          -   *\n"
           "*   ------------------------------   *\n"
           "*                                    *\n"
           "**************************************\n");
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   using namespace betterchain::contracts;

   get_account_results result;
   result.account_name = params.account_name;

   const auto& d = db.get_database();
   share_type  balance        = contracts::get_betterchain_balance(d, params.account_name );
   const auto& staked_balance = d.get<staked_balance_object,by_owner_name>( params.account_name );

   result.betterchain_balance          = asset(balance, BETTERCHAIN_SYMBOL);
