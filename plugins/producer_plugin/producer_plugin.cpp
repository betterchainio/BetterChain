...
#include <betterchain/net_plugin/net_plugin.hpp>
...
public:
   ...
   uint32_t _production_skip_flags              = betterchain::chain::skip_nothing;
...
void new_chain_banner(const betterchain::chain::chain_controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
      "*   ------ NEW CHAIN ------   *\n"
      "*   -  Welcome to BetterChain!  -   *\n"
      "*   -----------------------   *\n"
      "*                             *\n"
      "*******************************\n"
      "\n";
...
{ try {
   ...
   if (!my->_producers.empty())
   {
      ...
      if(my->_production_enabled)
      {
         ...
         my->_production_skip_flags |= betterchain::chain::skip_undo_history_check;
      }
...
block_production_condition::block_production_condition_enum producer_plugin_impl::maybe_produce_block(fc::mutable_variant_object& capture) {
   ...
   betterchain::chain::public_key_type scheduled_key = chain.get_producer(scheduled_producer).signing_key;
