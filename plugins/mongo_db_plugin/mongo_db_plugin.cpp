 ...
 */
#include <betterchain/mongo_db_plugin/mongo_db_plugin.hpp>
...
public:
   ...
   static abi_def betterchain_abi; // cached for common use
...
abi_def mongo_db_plugin_impl::betterchain_abi;
                ...
                const chain::action& msg)
  {
     ...
     try {
        ...
        if (msg.account == chain::config::system_account_name) {
           abis.set_abi(mongo_db_plugin_impl::betterchain_abi);
        } else {
        ...
        try {
           ...
           elog("Unable to convert BetterChain JSON to MongoDB JSON: ${e}", ("e", e.what()));
           elog("  BetterChain JSON: ${j}", ("j", json));
        }
     } catch (fc::exception& e) {
...
// For now providing some simple account processing to maintain betterchain_balance
void mongo_db_plugin_impl::update_account(const chain::action& msg) {
   ...
   if (msg.name == transfer) {
      ...
      asset from_balance = asset::from_string(from_account.view()["betterchain_balance"].get_utf8().value.to_string());
      asset to_balance = asset::from_string(to_account.view()["betterchain_balance"].get_utf8().value.to_string());
      from_balance -= chain::share_type(transfer.amount);
      to_balance += chain::share_type(transfer.amount);

      document update_from{};
      update_from << "$set" << open_document << "betterchain_balance" << from_balance.to_string()
                  << "updatedAt" << b_date{now}
                  << close_document;
      document update_to{};
      update_to << "$set" << open_document << "betterchain_balance" << to_balance.to_string()
                << "updatedAt" << b_date{now}
                << close_document;
      ...
      asset from_balance = asset::from_string(from_view["betterchain_balance"].get_utf8().value.to_string());
      from_balance -= newaccount.deposit;
      document update_from{};
      update_from << "$set" << open_document << "betterchain_balance" << from_balance.to_string() << close_document;
      ...
      doc << "name" << newaccount.name.to_string()
          << "betterchain_balance" << asset().to_string()
      ...
      asset balance = asset::from_string(from_account.view()["betterchain_balance"].get_utf8().value.to_string());
      asset unstack_balance = asset::from_string(from_account.view()["unstacking_balance"].get_utf8().value.to_string());
      ...
      document update_from{};
      update_from << "$set" << open_document
                  << "betterchain_balance" << balance.to_string()
                  << "unstacking_balance" << unstack_balance.to_string()
                  << "updatedAt" << b_date{now}
                  << close_document;
...
void mongo_db_plugin_impl::init() {
   ...
   betterchain_abi = chain::contracts::chain_initializer::betterchain_contract_abi();
   ...
   if (accounts.count(doc.view()) == 0) {
      ...
      doc << "name" << name(chain::config::system_account_name).to_string()
          << "betterchain_balance" << asset(chain::config::initial_token_supply).to_string()
          << "staked_balance" << asset().to_string()
          << "unstaking_balance" << asset().to_string()
          << "abi" << bsoncxx::from_json(fc::json::to_string(betterchain_abi))
          << "createdAt" << b_date{now}
          << "updatedAt" << b_date{now};
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         "The queue size between betterchaind and MongoDB plugin thread.")
         ("mongodb-uri,m", bpo::value<std::string>(),
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               " If not specified then plugin is disabled. Default database 'BetterChain' is used if not specified in URI.")
         ;
}
...
void mongo_db_plugin::plugin_initialize(const variables_map& options)
{
   if (options.count("mongodb-uri")) {
      ...
      wlog("betterchain::mongo_db_plugin configured, but no --mongodb-uri specified.");
