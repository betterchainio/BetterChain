...
BOOST_AUTO_TEST_CASE(test_k1) try {
   ...
   auto expected_public_key = std::string("BetterChain6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV");
...
BOOST_AUTO_TEST_CASE(test_r1) try {
   auto private_key_string = std::string("BetterChainR1iyQmnyPEGvFd8uffnk152WC2WryBjgTrg22fXQryuGL9mU6qW");
   auto expected_public_key = std::string("BetterChainR16EPHFSKVYHBjQgxVGQPrwCxTg7BbZ69H9i4gztN9deKTEXYne4");
   auto test_private_key = private_key(private_key_string);
   auto test_public_key = test_private_key.get_public_key();
