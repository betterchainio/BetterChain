...
namespace betterchain { namespace testing {
   ...
   tester::tester() {
     ...
     // uint64_t init_name = N(inita);
      ...
      for( uint32_t i = 0; i < config::producer_count; ++i ) {
         ...
         cfg.genesis.initial_accounts[i].liquid_balance     = asset::from_string( "1000000.0000 BetterChain" );
         cfg.genesis.initial_accounts[i].staking_balance    = asset::from_string( "1000000.0000 BetterChain" );
  ...
  void tester::set_tapos( signed_transaction& trx ) const {
   ...
   share_type tester::get_balance( const account_name& account ) const {
      ...
      return contracts::get_betterchain_balance(db, account);
   }
