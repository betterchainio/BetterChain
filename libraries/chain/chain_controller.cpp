...
#include <betterchain/chain/merkle.hpp>

#include <betterchain/chain/wasm_interface.hpp>

#include <betterchain/utilities/rand.hpp>
...
{ try {
   ...
   BETTERCHAIN_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
static void validate_shard_locks(const vector<shard_lock>& locks, const string& tag) {
   ...
   for (auto cur = locks.begin() + 1; cur != locks.end(); ++cur) {
      auto prev = cur - 1;
      BETTERCHAIN_ASSERT(*prev != *cur, block_lock_exception, "${tag} lock \"${a}::${s}\" is not unique", ("tag",tag)("a",cur->account)("s",cur->scope));
      BETTERCHAIN_ASSERT(*prev < *cur,  block_lock_exception, "${tag} locks are not sorted", ("tag",tag));
   }
}
      ...
      for (uint32_t cycle_index = 0; cycle_index < r.cycles_summary.size(); cycle_index++) {
         ...
         for (uint32_t shard_index = 0; shard_index < cycle.size(); shard_index++) {
            ...
            for (const auto& s: shard.read_locks) {
               BETTERCHAIN_ASSERT(write_locks.count(s) == 0, block_concurrency_exception,
            ...
            for (const auto& s: shard.write_locks) {
               ...
               BETTERCHAIN_ASSERT(read_locks.count(s) == 0, block_concurrency_exception,
            ...
            BETTERCHAIN_ASSERT(std::equal(used_read_locks.cbegin(), used_read_locks.cend(), shard.read_locks.begin()),
               block_lock_exception, "Read locks for executing shard: ${s} do not match those listed in the block", ("s", shard_index));
            BETTERCHAIN_ASSERT(std::equal(used_write_locks.cbegin(), used_write_locks.cend(), shard.write_locks.begin()),
   ...
   for (const auto& act : trx.actions ) {
      for (const auto& declared_auth : act.authorization) {
         if (!checker.satisfied(declared_auth)) {
            BETTERCHAIN_ASSERT(checker.satisfied(declared_auth), tx_missing_sigs,
                                            ...
                                            flat_set<account_name>    provided_accounts  )const
{
      ...
      for( const auto& declared_auth : act.authorization ) {
         ...
         if (min_permission_name) {
            ...
            if ((_skip_flags & skip_authority_check) == false) {
               ...
               BETTERCHAIN_ASSERT(get_permission(declared_auth).satisfies(min_permission, index),
                          tx_irrelevant_auth,
   ...
   if (!allow_unused_signatures && (_skip_flags & skip_transaction_signatures) == false)
      BETTERCHAIN_ASSERT(checker.all_keys_used(), tx_irrelevant_sig,
...
void chain_controller::validate_uniqueness( const signed_transaction& trx )const {
   ...
   BETTERCHAIN_ASSERT(transaction == nullptr, tx_duplicate, "transaction is not unique");
}
...
void chain_controller::validate_tapos(const transaction& trx)const {
   ...
   BETTERCHAIN_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), transaction_exception,
...
{ try {
   ...
   BETTERCHAIN_ASSERT( time_point(trx.expiration) <= now + fc::seconds(chain_configuration.max_transaction_lifetime),
              transaction_exception, "transaction expiration is too far in the future",
              ("trx.expiration",trx.expiration)("now",now)
              ("max_til_exp",chain_configuration.max_transaction_lifetime));
   BETTERCHAIN_ASSERT( now <= time_point(trx.expiration), transaction_exception, "transaction is expired",
...
void chain_controller::require_scope( const scope_name& scope )const {
   switch( uint64_t(scope) ) {
      case config::betterchain_all_scope:
      case config::betterchain_auth_scope:
...
const producer_object& chain_controller::validate_block_header(uint32_t skip, const signed_block& next_block)const {
   BETTERCHAIN_ASSERT(head_block_id() == next_block.previous, block_validate_exception, "",
              ("head_block_id",head_block_id())("next.prev",next_block.previous));
   BETTERCHAIN_ASSERT(head_block_time() < (fc::time_point)next_block.timestamp, block_validate_exception, "",
   ...
   if (next_block.block_num() % config::blocks_per_round != 0) {
      BETTERCHAIN_ASSERT(!next_block.new_producers, block_validate_exception,
   ...
   if(!(skip&skip_producer_signature))
      BETTERCHAIN_ASSERT(next_block.validate_signee(producer.signing_key), block_validate_exception,
   ...
   if(!(skip&skip_producer_schedule_check)) {
      BETTERCHAIN_ASSERT(next_block.producer == producer.owner, block_validate_exception,
...
ProducerRound chain_controller::calculate_next_round(const signed_block& next_block) {
   ...
   BETTERCHAIN_ASSERT(boost::range::equal(next_block.producer_changes, changes), block_validate_exception,
...
void chain_controller::update_last_irreversible_block()
{
   ...
   size_t offset = BETTERCHAIN_PERCENT(producer_objs.size(), config::percent_100- config::irreversible_threshold_percent);
