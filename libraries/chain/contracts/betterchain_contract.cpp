...
#include <betterchain/chain/permission_link_object.hpp>
#include <betterchain/chain/global_property_object.hpp>
#include <betterchain/chain/contracts/staked_balance_objects.hpp>
#include <betterchain/chain/contracts/producer_objects.hpp>
...
static void modify_betterchain_balance( apply_context& context, const account_name& account, share_type amt) {
...
share_type get_betterchain_balance( const chainbase::database& db, const account_name &account ) {
   ...
   FC_ASSERT(itr->value.size() == sizeof(share_type), "Invalid data in BetterChain balance table");
...
void apply_betterchain_newaccount(apply_context& context) { 
   ...
   context.require_write_lock( config::betterchain_auth_scope );

   BETTERCHAIN_ASSERT( validate(create.owner), action_validate_exception, "Invalid owner authority");
   BETTERCHAIN_ASSERT( validate(create.active), action_validate_exception, "Invalid active authority");
   BETTERCHAIN_ASSERT( validate(create.recovery), action_validate_exception, "Invalid recovery authority");

   auto& db = context.mutable_db;

   auto existing_account = db.find<account_object, by_name>(create.name);
   BETTERCHAIN_ASSERT(existing_account == nullptr, action_validate_exception,
   ...
   share_type creator_balance = get_betterchain_balance(context.db, create.creator);

   BETTERCHAIN_ASSERT(creator_balance >= create.deposit.amount, action_validate_exception,
              "Creator '${c}' has insufficient funds to make account creation deposit of ${a}",
              ("c", create.creator)("a", create.deposit));

   modify_betterchain_balance(context, create.creator, -create.deposit.amount);
...
/**
 *
 * @ingroup native_betterchain
 * @defgroup betterchain_betterchain_transfer betterchain::betterchain_transfer
 */
///@{
...
void apply_betterchain_transfer(apply_context& context) {
   ...
   try {
      BETTERCHAIN_ASSERT(transfer.amount > 0, action_validate_exception, "Must transfer a positive amount");
   ...
   try {
      auto& db = context.mutable_db;
      share_type from_balance = get_betterchain_balance(db, transfer.from);

      BETTERCHAIN_ASSERT(from_balance >= transfer.amount, action_validate_exception, "Insufficient Funds",
                 ("from_balance", from_balance)("transfer.amount",transfer.amount));

      modify_betterchain_balance(context, transfer.from, - share_type(transfer.amount) );
      modify_betterchain_balance(context, transfer.to,     share_type(transfer.amount) );
...
void apply_betterchain_lock(apply_context& context) {
   ...
   BETTERCHAIN_ASSERT(lock.amount > 0, action_validate_exception, "Locked amount must be positive");
   ...
   share_type locker_balance = get_betterchain_balance(context.db, lock.from);

   BETTERCHAIN_ASSERT( locker_balance >= lock.amount, action_validate_exception,
              "Account ${a} lacks sufficient funds to lock ${amt} BetterChain", ("a", lock.from)("amt", lock.amount)("available",locker_balance) );

   modify_betterchain_balance(context, lock.from, -share_type(lock.amount));
...
void apply_betterchain_unlock(apply_context& context) {
   auto unlock = context.act.as<contracts::unlock>();

   context.require_authorization(unlock.account);

   BETTERCHAIN_ASSERT(unlock.amount >= 0, action_validate_exception, "Unlock amount cannot be negative");

   const auto& balance = context.db.get<staked_balance_object, by_owner_name>(unlock.account);

   BETTERCHAIN_ASSERT(balance.staked_balance  >= unlock.amount, action_validate_exception,
...
void apply_betterchain_setcode(apply_context& context) {
   auto& db = context.mutable_db;
...
void apply_betterchain_setabi(apply_context& context) {
   auto& db = context.mutable_db;
...
void apply_betterchain_claim(apply_context& context) {
   auto claim = context.act.as<contracts::claim>();

   BETTERCHAIN_ASSERT(claim.amount > 0, action_validate_exception, "Claim amount must be positive");
   ...
   BETTERCHAIN_ASSERT(balance != nullptr, action_validate_exception,
   ...
   BETTERCHAIN_ASSERT(now >= balance_release_time, action_validate_exception,
              "Cannot claim balance until ${releaseDate}", ("releaseDate", balance_release_time));
   BETTERCHAIN_ASSERT(balance->unstaking_balance >= claim.amount, action_validate_exception,
   ...
   modify_betterchain_balance(context, claim.account, share_type(claim.amount));
}


void apply_betterchain_setproducer(apply_context& context) {
   auto update = context.act.as<setproducer>();
   context.require_authorization(update.name);
   BETTERCHAIN_ASSERT(update.name.good(), action_validate_exception, "Producer owner name cannot be empty");

   auto& db = context.mutable_db;
   auto producer = db.find<producer_object, by_owner>(update.name);

   if (producer) {
      BETTERCHAIN_ASSERT(producer->signing_key != update.key || producer->configuration != update.configuration,
...
void apply_betterchain_okproducer(apply_context& context) {
   auto approve = context.act.as<okproducer>();
   BETTERCHAIN_ASSERT(approve.approve == 0 || approve.approve == 1, action_validate_exception,
   ...
   BETTERCHAIN_ASSERT(voter.producer_votes.contains<producer_slate>(), action_validate_exception,
   ...
   BETTERCHAIN_ASSERT(slate.size < config::max_producer_votes, action_validate_exception,
              "Cannot approve producer; approved producer count is already at maximum");
   if (approve.approve)
      BETTERCHAIN_ASSERT(!slate.contains(producer.owner_name), action_validate_exception,
...
void apply_betterchain_setproxy(apply_context& context) {
...
void apply_betterchain_updateauth(apply_context& context) {
   context.require_write_lock( config::betterchain_auth_scope );

   auto update = context.act.as<updateauth>();
   BETTERCHAIN_ASSERT(!update.permission.empty(), action_validate_exception, "Cannot create authority with empty name");
   BETTERCHAIN_ASSERT(update.permission != update.parent, action_validate_exception,
              "Cannot set an authority as its own parent");
   BETTERCHAIN_ASSERT(validate(update.data), action_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.data));
   if (update.permission == "active")
      BETTERCHAIN_ASSERT(update.parent == "owner", action_validate_exception, "Cannot change active authority's parent from owner", ("update.parent", update.parent) );
   if (update.permission == "owner")
      BETTERCHAIN_ASSERT(update.parent.empty(), action_validate_exception, "Cannot change owner authority's parent");
   ...
   if (permission) {
      BETTERCHAIN_ASSERT(parent_id == permission->parent, action_validate_exception,
...
void apply_betterchain_deleteauth(apply_context& context) {
   auto remove = context.act.as<deleteauth>();
   BETTERCHAIN_ASSERT(remove.permission != "active", action_validate_exception, "Cannot delete active authority");
   BETTERCHAIN_ASSERT(remove.permission != "owner", action_validate_exception, "Cannot delete owner authority");
   ...
   { // Check for children
      ...
      BETTERCHAIN_ASSERT(range.first == range.second, action_validate_exception,
...
void apply_betterchain_linkauth(apply_context& context) {
   auto requirement = context.act.as<linkauth>();
   BETTERCHAIN_ASSERT(!requirement.requirement.empty(), action_validate_exception, "Required permission cannot be empty");
   ...
   if (link) {
      BETTERCHAIN_ASSERT(link->required_permission != requirement.requirement, action_validate_exception,
...
void apply_betterchain_unlinkauth(apply_context& context) {
   auto& db = context.mutable_db;
   ...
   auto link = db.find<permission_link_object, by_action_name>(link_key);
   BETTERCHAIN_ASSERT(link != nullptr, action_validate_exception, "Attempting to unlink authority, but no link found");
   if (context.controller.is_applying_block())
      db.remove(*link);
}
...
void apply_betterchain_onerror(apply_context& context) {
...
static const abi_serializer& get_abi_serializer() {
   ...
   if (!_abi_serializer) {
      _abi_serializer.emplace(chain_initializer::betterchain_contract_abi());
   }
...
static uint32_t get_next_sender_id(apply_context& context) {
   context.require_write_lock( config::betterchain_auth_scope );
   const auto& t_id = context.find_or_create_table(config::betterchain_auth_scope, config::system_account_name, N(deferred.seq));
   uint64_t key = N(config::betterchain_auth_scope);
...
void apply_betterchain_postrecovery(apply_context& context) {
   context.require_write_lock( config::betterchain_auth_scope );
...
void apply_betterchain_passrecovery(apply_context& context) {
...
void apply_betterchain_vetorecovery(apply_context& context) {
   context.require_write_lock( config::betterchain_auth_scope );
