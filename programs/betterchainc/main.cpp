 ...
 *  @defgroup betterchainclienttool BetterChain Command Line Client Reference
 *  @brief Tool for sending transactions and querying state from @ref betterchaind
 *  @ingroup betterchainclienttool
 */

/**
  @defgroup betterchainclienttool

  @section intro Introduction to BetterChainC

  `betterchainc` is a command line tool that interfaces with the REST api exposed by @ref betterchaind. In order to use `betterchainc` you will need to
  have a local copy of `betterchaind` running and configured to load the 'betterchain::chain_api_plugin'.

   betterchainc contains documentation for all of its commands. For a list of all commands known to betterchainc, simply run it with no arguments:
```
$ ./betterchainc
Command Line Interface to BetterChain Daemon
Usage: ./betterchainc [OPTIONS] SUBCOMMAND

Options:
  -h,--help                   Print this help actions and exit
  -H,--host TEXT=localhost    the host where betterchaind is running
  -p,--port UINT=8888         the port where betterchaind is running
  --wallet-host TEXT=localhost
                              the host where betterchain-walletd is running
  --wallet-port UINT=8888     the port where betterchain-walletd is running

Subcommands:
  create                      Create various items, on and off the blockchain
  get                         Retrieve various items and information from the blockchain
  set                         Set or update blockchain state
  transfer                    Transfer BetterChain from account to account
...
Usage: ./betterchainc create SUBCOMMAND

Subcommands:
  key                         Create a new keypair and print the public and private keys
  account                     Create a new account on the blockchain

$ ./betterchainc create account
Create a new account on the blockchain
Usage: ./betterchainc create account [OPTIONS] creator name OwnerKey ActiveKey
...
#include <betterchain/utilities/key_conversion.hpp>

#include <betterchain/chain/config.hpp>
#include <betterchain/chain_plugin/chain_plugin.hpp>
...
using namespace betterchain::utilities;
using namespace betterchain::client::help;
using namespace betterchain::client::localize;
using namespace betterchain::client::config;
using namespace boost::filesystem;

FC_DECLARE_EXCEPTION( explained_exception, 9000000, "explained exception, see error log" );
FC_DECLARE_EXCEPTION( localized_exception, 10000000, "an error occured" );
#define BetterChainC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
...
vector<chain::permission_level> get_account_permissions(const vector<string>& permissions) {
   auto fixedPermissions = permissions | boost::adaptors::transformed([](const string& p) {
      ...
      BetterChainC_ASSERT(pieces.size() == 2, "Invalid permission: ${p}", ("p", p));
...
betterchain::chain_apis::read_only::get_info_results get_info() {
  return call(host, port, get_info_func ).as<betterchain::chain_apis::read_only::get_info_results>();
}
...
void create_account(name creator, name newaccount, public_key_type owner, public_key_type active, bool sign, uint64_t staked_deposit) {
      auto owner_auth   = betterchain::chain::authority{1, {{owner, 1}}, {}};
      auto active_auth  = betterchain::chain::authority{1, {{active, 1}}, {}};
      auto recovery_auth = betterchain::chain::authority{1, {}, {{{creator, "active"}, 1}}};
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->set_callback([this] {
         ...
         if (is_delete) {
            ...
            authority auth;
            if (boost::istarts_with(authorityJsonOrFile, "BetterChain")) {
...
int main( int argc, char** argv ) {
   ...
   CLI::App app{"Command Line Interface to BetterChain Client"};
   app.require_subcommand();
   app.add_option( "-H,--host", host, localized("the host where betterchaind is running"), true );
   app.add_option( "-p,--port", port, localized("the port where betterchaind is running"), true );
   app.add_option( "--wallet-host", wallet_host, localized("the host where betterchain-walletd is running"), true );
   app.add_option( "--wallet-port", wallet_port, localized("the port where betterchain-walletd is running"), true );
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", betterchain::client::config::version_str)) << std::endl;
   });
   ...
   uint64_t amount;
   string memo;
   auto transfer = app.add_subcommand("transfer", localized("Transfer BetterChain from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending BetterChain"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving BetterChain"))->required();
   transfer->add_option("amount", amount, localized("The amount of BetterChain to send"))->required();
   ...
   benchmark_setup->set_callback([&]{
      ...
      BetterChainC_ASSERT( number_of_accounts > num_existing_accounts, fmter.str().c_str());
      ...
      if (num_existing_accounts == 0) {
      	BetterChainC_ASSERT( number_of_accounts >= 2, "must create at least 2 accounts" );
      }   
   ...
   try {
      ...
      if (errorString.find("Connection refused") != string::npos) {
         if (errorString.find(fc::json::to_string(port)) != string::npos) {
            std::cerr << localized("Failed to connect to betterchaind at ${ip}:${port}; is betterchaind running?", ("ip", host)("port", port)) << std::endl;
         } else if (errorString.find(fc::json::to_string(wallet_port)) != string::npos) {
            std::cerr << localized("Failed to connect to betterchain-walletd at ${ip}:${port}; is betterchain-walletd running?", ("ip", wallet_host)("port", wallet_port)) << std::endl;
         } else {
