const SNAPSHOT_FILE = 'https://github.com/betterchainio/BetterChain/programs/genesis/snapshot.csv';
let snapshot = null;
genesis = null;
...
class Genesis {
  ...
  format_initial_accounts() {
    this.json.initial_accounts = this.json.initial_accounts.map(account => {
      return {
        ...
        "owner_key": account.bbc.replace(/"/g, ''),
        "active_key": account.bbc.replace(/"/g, ''),
        "liquid_balance": `${account.balance.replace(/"/g, '')} BBC`
      };
    });
  }
  ...
  append_to_initial_accounts() {
    this.options.additional_accounts.map(account => {
      account.liquid_balance = `${account.liquid_balance} BBC`;
      return account;
    });
