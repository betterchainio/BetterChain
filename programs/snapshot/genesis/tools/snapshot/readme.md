...
- `cache` (true, false) Will cache successful fallbacks, including full paper trail: Block number, address, transaction, public key and generated BBC key. Can speed up the slow fallback method if running several times.
...
1. BBC Balance of an address must be greater-than or equal to one BBC to be considered for inclusion in Genesis block
2. An BBC key must be associated to an account either by registration or fallback
...
3. If an ethereum address sent BBC ERC20 tokens to the BBC Crowdsale or BBC Token contract within the block range of the snapshot, these tokens are allocated to the respective address. 
4. If an address has unclaimed BBC in the BBC Crowdsale contract within block range, these tokens are allocated to the respective address.
...
2. Compile list of every address that has ever had an BBC balance, for each address:
	...
	3. Validate
		...
		2. Validate BBC Key, if valid set `registered` to `true`
		3. If BBC key error, save error to column `register_error`
	...
	6. Fallback _if `balance is gte 1` and `register_error is not null`_
	   1. Fast Fallback
		   1. Check database for any outgoing transactions (or keys cache), if found...
	   			...
	   			2. Generate and validate BBC Key
	   			3. If found and valid, set `valid` to `true` otherwise save error into `fallback_error` column
		2. Slow fallback
			1. Obtain list of unique addresses with balance gte 1 BBC and no BBC key and save them as keys in index (redis is used as index)
    ...
    6. Test
    	1. Daily Totals from DB against daily totals from BBC Utility Contract, failure here would not fail the below tests, but would instead result in inaccurate unclaimed balances. Difficult problem to detect without this test. 
    ...
    7. Output
    	1. **snapshot.csv** - comma-delimited list of ETH addresses, BBC keys and Total Balances (user, key, balance respectively)
...
With the output, you can use the Genesis Generator to create a well-formed genesis.json file for the BBC platform. 
...
- Testnet snapshots will produce accurate output based on period by constraining all blockchain activity to that range. This means that some seemingly superfluous calculations are conducted for testnet
	...
	- BBC Key Registration is concluded by last registration within the block range. 
- Mainnet simplifies a few things. However, it would be recommended that a cutoff block be enforced to encourage network consensus (primarily for registration transactions)
	...
	- BBC key registration is inferred from state returned by `keys` map in Crowdsale Contract. 
