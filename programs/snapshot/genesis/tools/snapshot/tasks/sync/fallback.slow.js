module.exports = ( state, complete ) => {
  ...
  const update_address_key = (address, tx_hash, next_tx) => {
    pubkey_from_tx_hash(tx_hash, pubkey => {
      const bbc_key = util.misc.convert_ethpk_to_bbcpk(pubkey)
      if(util.misc.is_bbc_public_key(bbc_key))
        db.Wallets
          .update({
            bbc_key: bbc_key,
            fallback: true,
          ...
          .then( () => {
            if(config.cache)
              db.Keys.upsert({ address: address, tx_hash: tx_hash, public_key: pubkey, derived_bbc_key: bbc_key })
            // console.log(`BBC Key Generated: #${block_index} => ${tx_hash} => ${address} => ${pubkey} => ${bbc_key}`)
            redis.del(address);
            pks_found++
            next_tx()
          })
