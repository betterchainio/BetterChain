module.exports = (state, complete) => {
  ...
  const key = (wallet, finished) => {
    query.last_register(wallet.address, state.block_begin, state.block_end, bbc_key => {
      wallet.bbc_key = bbc_key
      finished( null, wallet )
    })
  }
  ...
  const transfers = (wallet, finished) => {
    ...
    //mainnet balance calculation uses BBC ERC20 token's balanceOf() method.
    ...
    const add = next => {
      ...
      query.address_transfers_in(wallet.address, state.block_begin, state.block_end)
        .then( results => {
          let _results = results.map( result => new bn(result.dataValues.bbc_amount) )
    ...
    const subtract = next => {
      query.address_transfers_out(wallet.address, state.block_begin, state.block_end)
        .then( results => {
          let _results = results.map( result => new bn(result.dataValues.bbc_amount).times(-1) )
          wallet.transfers = wallet.transfers.concat(_results)
          next()
        })
    }
  ...
  const reclaimables = ( wallet, finished ) => {
    query.address_reclaimables( wallet.address, state.block_begin, state.block_end )
      .then( results  => {
        if( results.length ) {
          wallet.reclaimables = results.map( reclaim => { return { address: wallet.address, value: reclaim.dataValues.bbc_amount } } )
        }
        finished( null, wallet )
      })
  }
  ...
  const log_table_reset = () => {
    ...
    table.setHeading('V', 'R', 'ETH', 'BBC', 'In Wallet', 'Unclaimed', 'Reclaimed', 'Total', 'Reg. Error')
  }
  ...
  const log_table_row = (wallet) => {
    table.addRow(
      ...
      wallet.address,
      wallet.bbc_key,
      `${wallet.balance.wallet} BBC`,
      `${wallet.balance.unclaimed} BBC`,
      `${wallet.balance.reclaimed} BBC`,
      `${wallet.balance.total} BBC`,
