...
const update_title_bar_progress = (stage = "") => {
  ...
  switch( stage ){
    ...
    case 'distribute':
      title = `BBC | Snapshot ${debug.rates.percent_complete}% | ${debug.registry.accepted+debug.registry.rejected}/${debug.registry.total}`
    break
    case 'registry':
      title = `BBC | Building Registry | ${registrants.length}`
    break
    case 'reclaimer':
      title = `BBC | Reclaimable TXs | ${ debug.reclaimable.total }`
    break
    case 'complete':
      title = `BBC | Snapshot Complete`
    break
    default:
      title = `BBC | Welcome`
    break
...
ui_refresh = setInterval( () => {
  ...
  if( disconnected ) {
    ...
    document.getElementsByClassName('disconnected')[0].innerHTML = `
      ...
      See Readme</a> for more information
      </p>
