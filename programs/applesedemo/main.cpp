...
void print_pub_for_key(SecKeyRef key) {
   ...
   cout << "public_key(BetterChainR1" << fc::to_base58(checksummed.data(), checksummed.size()) << ")" << endl;
}
...
void sign(const string& hex) {
   ...
   cout << "signature(BetterChainR1" << fc::to_base58(checksummed.data(), checksummed.size()) << ")" << endl;
