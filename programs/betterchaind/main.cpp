...
#include <betterchain/http_plugin/http_plugin.hpp>
...
{
   try {
      app().set_version(betterchain::betterchaind::config::version);
      if(!app().initialize<chain_plugin, http_plugin, net_plugin>(argc, argv))
         return -1;
      initialize_logging();
      ilog("betterchaind version ${ver}", ("ver", betterchain::betterchaind::config::itoh(static_cast<uint32_t>(app().version()))));
      app().startup();
      app().exec();
   } catch (const fc::exception& e) {
      elog("${e}", ("e",e.to_detail_string()));
