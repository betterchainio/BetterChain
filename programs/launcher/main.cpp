...
public:
  ...
  vector<betterchaind_def> instances;
...
public:
  ...
  betterchaind_def*       instance;
  string          gelf_endpoint;
};

void
betterchaind_def::mk_dot_label () {
  dot_label_str = name + "\\nprod=";
...
void
betterchaind_def::set_host( host_def* h ) {
  host = h->host_name;
  p2p_port = h->p2p_port();
...
struct server_identities {
  ...
  vector<string> db;
  string remote_betterchain_root;
  remote_deploy ssh;
};
...
struct launcher_def {
  ...
  string betterchaind_extra_args;
  testnet_def network;
  string gelf_endpoint;
  ...
  void assign_name (betterchaind_def &node);
  ...
  void prep_remote_config_dir (betterchaind_def &node, host_def *host);
  void launch (betterchaind_def &node, string &gts);
  void kill (launch_modes mode, string sig_opt);
  pair<host_def, betterchaind_def> find_node(uint16_t node_num);
  vector<pair<host_def, betterchaind_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cli) {
  cli.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "BetterChainD does not require transaction signatures.")
    ("betterchaind", bpo::value<string>(&betterchaind_extra_args), "forward betterchaind command line argument(s) to each instance of betterchaind, enclose arg in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many betterchaind instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &betterchaind : binding.instances) {
          betterchaind.host = binding.host_name;
          betterchaind.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(betterchaind.p2p_port);
          aliases.push_back (betterchaind.name);
        }
      }
  ...
  char* erd_env_var = getenv ("BETTERCHAIN_ROOT_DIR");
...
void
launcher_def::assign_name (betterchaind_def &node) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.betterchain_root_dir = erd;
    local_host.genesis = genesis.string();

    for (size_t i = 0; i < total_nodes; i++) {
      betterchaind_def betterchaind;

      assign_name(betterchaind);
      aliases.push_back(betterchaind.name);
      betterchaind.set_host (&local_host);
      local_host.instances.emplace_back(move(betterchaind));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      if (ph_count == 0) {
        ...
        lhost->betterchain_root_dir =
          (local_id.contains (lhost->host_name) || servers.remote_betterchain_root.empty()) ?
          erd : servers.remote_betterchain_root;
      ...
      assign_name(betterchaind);
      betterchaind.has_db = false;

      if (servers.db.size()) {
        for (auto &dbn : servers.db) {
          if (lhost->host_name == dbn) {
            betterchaind.has_db = true;
            break;
          }
        }
      }
      aliases.push_back(betterchaind.name);
      betterchaind.set_host (lhost);
      lhost->instances.emplace_back(move(betterchaind));
      --ph_count;
    }
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  betterchaind_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bf::path dd = bf::path(host->betterchain_root_dir) / instance.data_dir;
    if (bf::exists (dd)) {
      if (force_overwrite) {
    ...
    bf::path dpath = bf::path (host->betterchain_root_dir) / instance.data_dir / "config.ini";
    ...
    dpath = bf::path (host->betterchain_root_dir) / instance.data_dir / "logging.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = betterchain::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = betterchain::mongo_db_plugin\n";
  }
  cfg << "plugin = betterchain::chain_api_plugin\n"
      << "plugin = betterchain::account_history_api_plugin\n";
  cfg.close();
}
...
void
launcher_def::prep_remote_config_dir (betterchaind_def &node, host_def *host) {
  bf::path abs_data_dir = bf::path(host->betterchain_root_dir) / node.data_dir;
  string add = abs_data_dir.string();
  string cmd = "cd " + host->betterchain_root_dir;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->betterchain_root_dir
...
void
launcher_def::launch (betterchaind_def &instance, string &gts) {
  ...
  bf::path pidf  = dd / "betterchaind.pid";

  host_def* host = deploy_config_files (*instance.node);
  node_rt_info info;
  info.remote = !host->is_local();

  string betterchaindcmd = "programs/betterchaind/betterchaind ";
  if (skip_transaction_signatures) {
    betterchaindcmd += "--skip-transaction-signatures ";
  }
  if (!betterchaind_extra_args.empty()) {
    betterchaindcmd += betterchaind_extra_args + " ";
  }

  if( add_enable_stale_production ) {
    betterchaindcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  betterchaindcmd += "--data-dir " + instance.data_dir;
  if (gts.length()) {
    betterchaindcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->betterchain_root_dir + "; nohup " + betterchaindcmd + " > "
    ...
    string cmd = "cd " + host->betterchain_root_dir + "; kill -9 `cat " + pidf.string() + "`";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << betterchaindcmd << endl;

    bp::child c(betterchaindcmd, bp::std_out > reout, bp::std_err > reerr );
    bf::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << betterchaindcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(betterchaind_def, string sig_opt) {
}
#endif
...
pair<host_def, betterchaind_def>
...
vector<pair<host_def, betterchaind_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, betterchaind_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const betterchaind_def& node = node_pair.second;
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.betterchain_root_dir + "; "
                 + "export BETTERCHAIN_HOME=" + host.betterchain_root_dir + string("; ")
                 + "export BETTERCHAIN_TN_NODE=" + node_num + "; "
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.betterchain_root_dir + "; "
                 + "export BETTERCHAIN_HOME=" + host.betterchain_root_dir + "; "
                 + "export BETTERCHAIN_TN_NODE=" + node_num + "; "
                 + "export BETTERCHAIN_TN_RESTART_DATA_DIR=" + node.data_dir + "; "
                 + "./scripts/tn_down.sh";
...
int main (int argc, char *argv[]) {
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << betterchain::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(betterchain_root_dir)
            (host_name)(public_name)
            (base_p2p_port)(base_http_port)(def_file_size)
            (instances) )

FC_REFLECT( betterchaind_def,
...
FC_REFLECT( server_identities, (producer) (observer) (db) (remote_betterchain_root) (ssh) )
