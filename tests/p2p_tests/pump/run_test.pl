...
my $betterchain_home = defined $ENV{BETTERCHAIN_HOME} ? $ENV{BETTERCHAIN_HOME} : getcwd;
my $betterchaind = $betterchain_home . "/programs/betterchaind/betterchaind";
my $betterchainc = $betterchain_home . "/programs/betterchainc/betterchainc";

my $nodes = defined $ENV{BETTERCHAIN_TEST_RING} ? $ENV{BETTERCHAIN_TEST_RING} : "1";
my $pnodes = defined $ENV{BETTERCHAIN_TEST_PRODUCERS} ? $ENV{BETTERCHAIN_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$betterchain_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of betterchaind instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"BetterChain6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = betterchain::producer_plugin\n";
        print $cfg "plugin = betterchain::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$betterchainc transfer betterchain inita 10 >> betterchainc.out 2>> betterchainc.err`;
            $counter++;
            if ($counter % 1000 == 0) {
