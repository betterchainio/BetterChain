...
BOOST_FIXTURE_TEST_CASE(trx_variant, testing_fixture) {
   ...
   betterchain::chain::processed_transaction trx;
...
BOOST_FIXTURE_TEST_CASE(irrelevant_auth, testing_fixture) {
   ...
   transaction_emplace_message(trx, config::betterchain_contract_name, vector<types::account_permission>{{"inita", "active"}},
...
{ try {
      auto lag = betterchain_percent(config::blocks_per_round, config::irreversible_threshold_percent);
      {
