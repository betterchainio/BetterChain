...
#include <betterchain/chain/producer_objects.hpp>
...
BOOST_FIXTURE_TEST_CASE(producer_voting_1, testing_fixture) {
   try {
...

      {
         ...
         const auto& joeVotes = chain_db.get<betterchain::chain::producer_votes_object, betterchain::chain::by_owner_name>("joe");
   ...
   try {
      ...
      auto run = [this](std::vector<Action> actions) {
...

         {
            ...
            const auto& producerVotes = chain_db.get<betterchain::chain::producer_votes_object, betterchain::chain::by_owner_name>("producer");
...
BOOST_FIXTURE_TEST_CASE(auth_links, testing_fixture) { try {
   ...
   chain.produce_blocks();
   Link_Authority(chain, alice, "spending", betterchain, "transfer");
   BOOST_CHECK_EQUAL(
           (chain_db.find<permission_link_object, by_message_type>(boost::make_tuple("alice", "betterchain", "transfer"))),
            nullptr);
   chain.produce_blocks();

   {
      auto obj = chain_db.find<permission_link_object, by_message_type>(boost::make_tuple("alice", "betterchain", "transfer"));
      BOOST_CHECK_NE(obj, nullptr);
      BOOST_CHECK_EQUAL(obj->account, "alice");
      BOOST_CHECK_EQUAL(obj->code, "betterchain");
   ...
   BOOST_CHECK_THROW(chain.review_transaction([&chain, &backup](signed_transaction& trx, auto) {
                        trx.messages.front().authorization = {{"alice", "scud"}};
                        trx.scope = {"alice", config::betterchain_contract_name};
                        chain.sign_transaction(trx);
                        backup = trx;
                        return true;
                     }), tx_irrelevant_auth);
   // Now set the default authority to scud...
   Link_Authority(chain, alice, "scud", betterchain);
