...
using rate_limiting_type = betterchain::chain::testing_blockchain::rate_limit_type;
...
{ try {
   ...
   auto rate = betterchain::chain::chain_controller::_transaction_message_rate(now, last_update_sec, rate_limit_time_frame_sec,
                                                                       rate_limit, previous_rate, account_type, N(my.name));
   BOOST_CHECK_EQUAL(10, rate);

   previous_rate = 10;
   BOOST_CHECK_EXCEPTION(betterchain::chain::chain_controller::_transaction_message_rate(now, last_update_sec, rate_limit_time_frame_sec,
                                                                                 rate_limit, previous_rate, account_type, N(my.name)),\
                         tx_msgs_exceeded,
   ...
   rate = betterchain::chain::chain_controller::_transaction_message_rate(now, last_update_sec, rate_limit_time_frame_sec,
                                                                  rate_limit, previous_rate, account_type, N(my.name));
   BOOST_CHECK_EQUAL(10, rate);
...
{ try {
   ...
   BOOST_REQUIRE_EXCEPTION(betterchain::chain::chain_controller::_transaction_message_rate(now, last_update_sec, rate_limit_time_frame_sec,
                                                                                   rate_limit, rate, account_type, N(my.name)),\
                           tx_msgs_exceeded,
                           [](tx_msgs_exceeded const & e) -> bool { return true; } );
