...
#define BETTERCHAIN_REQUIRE_THROW( expr, exc_type )          \
{                                                         \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "BETTERCHAIN_REQUIRE_THROW begin "             \
         << req_throw_info << std::endl;                  \
   BOOST_REQUIRE_THROW( expr, exc_type );                 \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "BETTERCHAIN_REQUIRE_THROW end "               \
         << req_throw_info << std::endl;                  \
}

#define BETTERCHAIN_CHECK_THROW( expr, exc_type )            \
{                                                         \
   std::string req_throw_info = fc::json::to_string(      \
      fc::mutable_variant_object()                        \
      ("source_file", __FILE__)                           \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "BETTERCHAIN_CHECK_THROW begin "               \
         << req_throw_info << std::endl;                  \
   BOOST_CHECK_THROW( expr, exc_type );                   \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "BETTERCHAIN_CHECK_THROW end "                 \
         << req_throw_info << std::endl;                  \
}

namespace betterchain { namespace chain {
FC_DECLARE_EXCEPTION(testing_exception, 6000000, "test framework exception")
FC_DECLARE_DERIVED_EXCEPTION(missing_key_exception, betterchain::chain::testing_exception, 6010000, "key could not be found")
