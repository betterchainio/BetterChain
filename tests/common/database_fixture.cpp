...
#include <betterchain/producer_plugin/producer_plugin.hpp>

#include <betterchain/utilities/tempdir.hpp>

#include <betterchain/native_contract/native_contract_chain_initializer.hpp>
#include <betterchain/native_contract/native_contract_chain_administrator.hpp>
#include <betterchain/native_contract/objects.hpp>
...
uint32_t BETTERCHAIN_TESTING_GENESIS_TIMESTAMP = 1431700005;

namespace betterchain { namespace chain {

testing_fixture::testing_fixture() {
   default_genesis_state.initial_timestamp = fc::time_point_sec(BETTERCHAIN_TESTING_GENESIS_TIMESTAMP);
...
private_key_type testing_fixture::get_private_key(const public_key_type& public_key) const {
   ...
   BETTERCHAIN_ASSERT(itr != key_ring.end(), missing_key_exception,
...
testing_blockchain::testing_blockchain(chainbase::database& db, fork_database& fork_db, block_log& blocklog,
   ...
   : chain_controller(db, fork_db, blocklog, initializer, native_contract::make_administrator(),
                      ::betterchain::chain_plugin::default_transaction_execution_time * 1000,
                      ::betterchain::chain_plugin::default_received_block_transaction_execution_time * 1000,
                      ::betterchain::chain_plugin::default_create_block_transaction_execution_time * 1000,
...
types::asset testing_blockchain::get_liquid_balance(const types::account_name& account) {
   return get_database().get<balance_object, betterchain::chain::by_owner_name>(account).balance;
}

types::asset testing_blockchain::get_staked_balance(const types::account_name& account) {
   return get_database().get<staked_balance_object, betterchain::chain::by_owner_name>(account).staked_balance;
}

types::asset testing_blockchain::get_unstaking_balance(const types::account_name& account) {
   return get_database().get<staked_balance_object, betterchain::chain::by_owner_name>(account).unstaking_balance;
}
