...
#define MKACCT_IMPL(chain, name, creator, active, owner, recovery, deposit) \
   { \
      ...
      trx.scope = sort_names({ #creator, config::betterchain_contract_name }); \
      transaction_emplace_message(trx, config::betterchain_contract_name, \
...
#define SETCODE3(chain, acct, wast) \
   { \
      auto wasm = betterchain::chain::wast_to_wasm(wast); \
      types::setcode handler; \
      handler.account = #acct; \
      handler.code.assign(wasm.begin(), wasm.end()); \
      betterchain::chain::signed_transaction trx; \
      trx.scope = sort_names({config::betterchain_contract_name, #acct}); \
      transaction_emplace_message(trx, config::betterchain_contract_name, vector<types::account_permission>{{#acct,"active"}}, \
                                  "setcode", handler); \
   }
...
#define STAKE4(chain, sender, recipient, amount) \
   { \
      betterchain::chain::signed_transaction trx; \
      trx.scope = sort_names( { #sender, #recipient, config::betterchain_contract_name } ); \
      transaction_emplace_message(trx, config::betterchain_contract_name, vector<types::account_permission>{{#sender, "active"}}, \
...
#define BEGIN_UNSTAKE3(chain, account, amount) \
   { \
      betterchain::chain::signed_transaction trx; \
      trx.scope = sort_names( { config::betterchain_contract_name } ); \
      transaction_emplace_message(trx, config::betterchain_contract_name, \
                         vector<types::account_permission>{{#account, "active"}}, \
...
#define FINISH_UNSTAKE3(chain, account, amount) \
   { \
      betterchain::chain::signed_transaction trx; \
      trx.scope = sort_names( { config::betterchain_contract_name, #account } ); \
      transaction_emplace_message(trx, config::betterchain_contract_name, vector<types::account_permission>{{#account, "active"}}, \
...
#define MKPDCR4(chain, owner, key, cfg) \
   { \
      betterchain::chain::signed_transaction trx; \
      trx.scope = sort_names( {#owner, config::betterchain_contract_name} ); \
      transaction_emplace_message(trx, config::betterchain_contract_name, \
...
#define APPDCR4(chain, voter, producer, approved) \
   { \
      betterchain::chain::signed_transaction trx; \
      trx.scope = sort_names( {#voter, config::betterchain_contract_name} ); \
      transaction_emplace_message(trx, config::betterchain_contract_name,  \
