...
#include <betterchain/chain/key_value_object.hpp>
...
#include <betterchain/chain/staked_balance_objects.hpp>
#include <betterchain/chain/balance_object.hpp>
...
uint32_t CallFunction( testing_blockchain& chain, const types::message& msg, const vector<char>& data, const vector<account_name>& scope = {N(testapi)}) {
   ...
   betterchain::chain::signed_transaction trx;
   trx.scope = scope;
...
void send_set_code_message(testing_blockchain& chain, types::setcode& handler, account_name account)
{
   ...
   trx.messages[0].code = config::betterchain_contract_name;
...
{ try {
      //auto wasm = assemble_wast( readFile2("/home/matu/Documents/Dev/betterchain/build/contracts/test_api/test_api.wast").c_str() );
...
BOOST_FIXTURE_TEST_CASE(test_page_memory_exceeded, testing_fixture)
{
   ...
   catch (betterchain::chain::page_memory_error& )
   {
      // expected behavior
   }
}
...
{ try { \
   ...
   Make_Blockchain(chain, 5000, \
         ::betterchain::chain_plugin::default_received_block_transaction_execution_time, \
         ::betterchain::chain_plugin::default_create_block_transaction_execution_time, chain_controller::txn_msg_limits {}); \
   chain.produce_blocks(2); \
   Make_Account(chain, testapi); \
...
{ try {
   ...
   transaction_emplace_message(trx, "betterchain",
   ...
   const auto& balance        = chain_db.get<betterchain::chain::balance_object,betterchain::chain::by_owner_name>( "acc1" );
   const auto& staked_balance = chain_db.get<betterchain::chain::staked_balance_object,betterchain::chain::by_owner_name>( "acc1" );

   // manually set balance so it can be verified in test_account
   chain_db.modify(balance, [](betterchain::chain::balance_object& bo) {
      bo.balance = 24;
   });
   chain_db.modify(staked_balance, [](betterchain::chain::staked_balance_object& sbo) {
