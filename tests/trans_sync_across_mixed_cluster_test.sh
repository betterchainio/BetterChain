...
getHeadBlockNum()
{
  INFO="$(programs/betterchainc/betterchainc get info)"
  verifyErrorCode "betterchainc get info"
...
# stand up betterchaind cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --betterchaind \"--plugin betterchain::wallet_api_plugin\" $launcherOpts
programs/launcher/launcher --betterchaind "--plugin betterchain::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/betterchainc/betterchainc --port $port2 get block 1)"
    verifyErrorCode "betterchainc get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/betterchainc/betterchainc create key)"
verifyErrorCode "betterchainc create key"
...
PASSWORD_INITA="$(programs/betterchainc/betterchainc wallet create --name inita)"
verifyErrorCode "betterchainc wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/betterchainc/betterchainc wallet import --name inita $INITA_PRV_KEY
verifyErrorCode "betterchainc wallet import"
programs/betterchainc/betterchainc wallet import --name inita $PRV_KEY1
verifyErrorCode "betterchainc wallet import"
programs/betterchainc/betterchainc wallet import --name inita $PRV_KEY2
verifyErrorCode "betterchainc wallet import"
programs/betterchainc/betterchainc wallet import --name inita $PRV_KEY3
verifyErrorCode "betterchainc wallet import"
...
ACCOUNT_INFO="$(programs/betterchainc/betterchainc create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "betterchainc create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/betterchainc/betterchainc get account testera)"
verifyErrorCode "betterchainc get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/betterchainc/betterchainc transfer inita testera 975321 "test transfer")"
    verifyErrorCode "betterchainc transfer"
	...
	TRANS_INFO="$(programs/betterchainc/betterchainc --port $port2 get transaction $TRANS_ID)"
	verifyErrorCode "betterchainc get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
