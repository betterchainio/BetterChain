...
class Utils:
    ...
    BetterChainServerName="betterchaind"
    BetterChainClientPath="programs/betterchainc/betterchainc"
    BetterChainWalletPath="programs/betterchain-walletd/betterchain-walletd"
    BetterChainServerName="betterchaind"
    BetterChainServerPath="programs/betterchaind/%s" % (BetterChainServerName)

    BetterChainLauncherPath="programs/launcher/launcher"
    MongoPath="mongo"
    ...
    # mongoSyncTime: betterchaind mongodb plugin seems to sync with a 10-15 seconds delay. This will inject
    ...
    def iAmNotNoon():
        Utils.amINoon=False
        Utils.BetterChainServerName="betterchaind"
        Utils.BetterChainClientPath="programs/betterchainc/betterchainc"
        Utils.BetterChainWalletPath="programs/betterchain-walletd/betterchain-walletd"
        Utils.BetterChainServerPath="programs/betterchaind/%s" % (Utils.BetterChainServerName)
...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, alive=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="BetterChaintest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def getBlock(self, blockNum, retry=True, silentErrors=False):
        if not self.enableMongo:
            cmd="%s %s get block %s" % (Utils.BetterChainClientPath, self.endpointArgs, blockNum)
    ...
    def getTransaction(self, transId, retry=True, silentErrors=False):
        if not self.enableMongo:
            cmd="%s %s get transaction %s" % (Utils.BetterChainClientPath, self.endpointArgs, transId)
            Utils.Debug and Utils.Print("cmd: %s" % (cmd))
            try:
                trans=Node.runCmdReturnJson(cmd)
                return trans
    ...
    def createAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False):
        ...
        if Utils.amINoon:
            cmd="%s %s create account --staked-deposit %d %s %s %s %s" % (
                Utils.BetterChainClientPath, self.endpointArgs, stakedDeposit, creatorAccount.name, account.name,
                account.ownerPublicKey, account.activePublicKey)
        else:
            cmd="%s %s create account %s %s %s %s" % (Utils.BetterChainClientPath, self.endpointArgs,
    ...
    def getBetterChainAccount(self, name):
        cmd="%s %s get account %s" % (Utils.BetterChainClientPath, self.endpointArgs, name)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def getBetterChainAccountFromDb(self, name):
        cmd="%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
    ...
    def verifyAccount(self, account):
        if not self.enableMongo:
            ret=self.getBetterChainAccount(account.name)
            if ret is not None:
            ...
            for i in range(2):
                ret=self.getBetterChainAccountFromDb(account.name)
                if ret is not None:
                    stakedBalance=ret["staked_balance"]
                    if stakedBalance is None:
                        Utils.Print("ERROR: Failed to verify account creation.", account.name)
    ...
    def transferFunds(self, source, destination, amount, memo="memo", force=False):
        cmd="%s %s transfer %s %s %d" % (
            Utils.BetterChainClientPath, self.endpointArgs, source.name, destination.name, amount)
        cmdArr=cmd.split()
        cmdArr.append(memo)
        if force:
            cmdArr.append("-f")
    ...
    def getAccountsByKey(self, key):
        cmd="%s %s get accounts %s" % (Utils.BetterChainClientPath, self.endpointArgs, key)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def getServants(self, name):
        cmd="%s %s get servants %s" % (Utils.BetterChainClientPath, self.endpointArgs, name)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def getAccountBalance(self, name):
        if not self.enableMongo:
            account=self.getBetterChainAccount(name)
            field=account["betterchain_balance"]
            ...
            account=self.getBetterChainAccountFromDb(name)
    ...
    def getTransactionsByAccount(self, name):
        cmd="%s %s get transactions %s" % (Utils.BetterChainClientPath, self.endpointArgs, name)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def getAccountCodeHash(self, account):
        cmd="%s %s get code %s" % (Utils.BetterChainClientPath, self.endpointArgs, account)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
            retStr=Node.__checkOutput(cmd.split())
    ...
    def publishContract(self, account, wastFile, abiFile, waitForTransBlock=False, shouldFail=False):
        cmd="%s %s set contract %s %s %s" % (Utils.BetterChainClientPath, self.endpointArgs, account, wastFile, abiFile)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        trans=None
        try:
            trans=Node.runCmdReturnJson(cmd)
    ...
    def createProducer(self, account, ownerPublicKey, waitForTransBlock=False):
        cmd="%s %s create producer %s %s" % (Utils.BetterChainClientPath, self.endpointArgs, account, ownerPublicKey)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        trans=None
        try:
            trans=Node.runCmdReturnJson(cmd)
    ...
    def getTable(self, account, contract, table):
        cmd="%s %s get table %s %s %s" % (Utils.BetterChainClientPath, self.endpointArgs, account, contract, table)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def pushMessage(self, contract, action, data, opts):
        cmd=None
        if Utils.amINoon:
            cmd="%s %s push action %s %s" % (Utils.BetterChainClientPath, self.endpointArgs, contract, action)
        else:
            cmd="%s %s push message %s %s" % (Utils.BetterChainClientPath, self.endpointArgs, contract, action)
        cmdArr=cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False):
        cmd="%s %s set action permission %s %s %s %s" % (
            Utils.BetterChainClientPath, self.endpointArgs, account, code, pType, requirement)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        trans=None
        try:
            trans=Node.runCmdReturnJson(cmd)
    ...
    def getInfo(self, silentErrors=False):
        cmd="%s %s get info" % (Utils.BetterChainClientPath, self.endpointArgs)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
...
class WalletMgr(object):
    ...
    def __init__(self, walletd, betterchaindPort=8888, betterchaindHost="localhost", port=8899, host="localhost"):
        self.walletd=walletd
        self.betterchaindPort=betterchaindPort
        self.betterchaindHost=betterchaindHost
        self.port=port
        self.host=host
        self.wallets={}
        self.__walletPid=None
        self.endpointArgs="--host %s --port %d" % (self.betterchaindHost, self.betterchaindPort)
        if self.walletd:
    ...
    def launch(self):
        ...
        cmd="%s --data-dir %s --http-server-address=%s:%d" % (
            Utils.BetterChainWalletPath, WalletMgr.__walletDataDir, self.host, self.port)
    ...
    def create(self, name):
        ...
        if wallet is not None:
            ...
            return wallet
        p = re.compile('\n\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet create --name %s" % (Utils.BetterChainClientPath, self.endpointArgs, name)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
    ...
    def importKey(self, account, wallet):
        ...
        cmd="%s %s wallet import --name %s %s" % (
            Utils.BetterChainClientPath, self.endpointArgs, wallet.name, account.ownerPrivateKey)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
        ...
        if account.activePrivateKey is None:
            ...
            cmd="%s %s wallet import --name %s %s" % (
                Utils.BetterChainClientPath, self.endpointArgs, wallet.name, account.activePrivateKey)
            Utils.Debug and Utils.Print("cmd: %s" % (cmd))
            try:
                retStr=subprocess.check_output(cmd.split(), stderr=subprocess.STDOUT).decode("utf-8")
            except subprocess.CalledProcessError as ex:
    ...
    def lockWallet(self, wallet):
        cmd="%s %s wallet lock --name %s" % (Utils.BetterChainClientPath, self.endpointArgs, wallet.name)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
    ...
    def unlockWallet(self, wallet):
        cmd="%s %s wallet unlock --name %s" % (Utils.BetterChainClientPath, self.endpointArgs, wallet.name)
        #Utils.Debug and Utils.Print("cmd: %s" % (cmd))
    ...
    def lockAllWallets(self):
        cmd="%s %s wallet lock_all" % (Utils.BetterChainClientPath, self.endpointArgs)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
    ...
    def getOpenWallets(self):
        ...
        p = re.compile('\s+\"(\w+)\s\*\",?\n', re.MULTILINE)
        cmd="%s %s wallet list" % (Utils.BetterChainClientPath, self.endpointArgs)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        retStr=subprocess.check_output(cmd.split()).decode("utf-8")
    ...
    def getKeys(self):
        ...
        p = re.compile('\n\s+\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet keys" % (Utils.BetterChainClientPath, self.endpointArgs)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        retStr=subprocess.check_output(cmd.split()).decode("utf-8")
        #Utils.Print("retStr: %s" % (retStr))
        m=p.findall(retStr)
...
class Cluster(object):
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=8899, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="BetterChaintest"):
        self.accounts={}
        self.nodes={}
    ...
    def launch(self, pnodes=1, total_nodes=1, topo="mesh", delay=1):
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.BetterChainServerName))
            return True
        ...
        cmd="%s -p %s -n %s -s %s -d %s" % (
            Utils.BetterChainLauncherPath, pnodes, total_nodes, topo, delay)
        cmdArr=cmd.split()
        if not self.walletd or self.enableMongo:
            if Utils.amINoon:
                cmdArr.append("--betterchaind")
            else:
                cmdArr.append("--betterchaind")
            if not self.walletd:
                cmdArr.append("--plugin betterchain::wallet_api_plugin")
            if self.enableMongo:
                if Utils.amINoon:
                    cmdArr.append("--plugin betterchain::mongo_db_plugin --mongodb-uri %s" % self.mongoUri)
                else:
                    cmdArr.append("--plugin betterchain::db_plugin --mongodb-uri %s" % self.mongoUri)
        ...
        if total_nodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.BetterChainServerName, total_nodes, len(nodes)))
            return False
    ...
    def createAccountKeys(count):
        ...
        for i in range(0, count):
            try:
                cmd="%s create key" % (Utils.BetterChainClientPath)
                Utils.Debug and Utils.Print("cmd: %s" % (cmd))
    ...
    def spreadFunds(self, amount=1):
        ...
        to=self.accounts[0]
        Utils.Print("Transfer %d units from account %s to %s on betterchain server port %d" % (
        ...
        nextBetterChainIdx=-1
        for i in range(0, count):
            account=self.accounts[i]
            nextInstanceFound=False
            for n in range(0, count):
                #Utils.Print("nextBetterChainIdx: %d, n: %d" % (nextBetterChainIdx, n))
                nextBetterChainIdx=(nextBetterChainIdx + 1)%count
                if self.nodes[nextBetterChainIdx].alive:
                    #Utils.Print("nextBetterChainIdx: %d" % (nextBetterChainIdx))
            ...
            #Utils.Print("nextBetterChainIdx: %d, count: %d" % (nextBetterChainIdx, count))
            node=self.nodes[nextBetterChainIdx]
        ...
        for node in self.nodes:
            if node.alive:
                Utils.Debug and Utils.Print("Validate funds on %s server port %d." %
                                            (Utils.BetterChainServerName, node.port))
                if node.validateSpreadFundsOnNode(Cluster.initaAccount, self.accounts, expectedTotal) is False:
                    Utils.Print("ERROR: Failed to validate funds on betterchain node port: %d" % (node.port))
                    return False
    ...
    # Populates list of BetterChainInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes):
        nodes=[]

        try:
            cmd="pgrep -a %s" % (Utils.BetterChainServerName)
            Utils.Debug and Utils.Print("cmd: %s" % (cmd))
            ...
            for i in range(0, totalNodes):
                ...
                if m is None:
                    Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.BetterChainServerName, pattern))
                    break
    ...
    # Check state of running betterchaind process and update BetterChainInstanceInfos
    #def updateBetterChainInstanceInfos(betterchainInstanceInfos):
    def updateNodesStatus(self):
        for node in self.nodes:
            node.checkPulse()

    # Kills a percentange of BetterChain instances starting from the tail and update betterchainInstanceInfos state
    def killSomeBetterChainInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.BetterChainServerName, killSignal))
    ...
    def relaunchBetterChainInstances(self):
    ...
    def killall(self):
        cmd="%s -k 15" % (Utils.BetterChainLauncherPath)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            Utils.Print("Launcher failed to shut down betterchain cluster.")
