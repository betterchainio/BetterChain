...
parser.add_argument("-h", "--host", type=str, help="%s host name" % (testUtils.Utils.BetterChainServerName),
                    default=LOCAL_HOST)
parser.add_argument("-p", "--port", type=int, help="%s host port" % testUtils.Utils.BetterChainServerName,
...
try:
    ...
    if walletMgr.launch() is False:
        ...
        errorExit("Failed to stand up betterchain walletd.")

    testWalletName="test"
    Print("Creating wallet \"%s\"." % (testWalletName))
    testWallet=walletMgr.create(testWalletName)
    if testWallet is None:
        cmdError("betterchain wallet create")
    ...
    if not enableMongo:
        ...
        account=node.getBetterChainAccountFromDb(currencyAccount.name)
...
# TODO Exchange contract currently not working on betterchain-noon
    if not amINoon:
...
# TODO remove failed on betterchain-noon
    if not amINoon:
    ...
    # approve producer
    # INFO="$(programs/betterchainc/betterchainc --host $SERVER --port $PORT --wallet-port 8899 set producer inita testera approve)"
    # verifyErrorCode "betterchainc approve producer"

    Print("Get account inita")
    account=node.getBetterChainAccount(initaAccount.name)
    if account is None:
    ...
    # unapprove producer
    # INFO="$(programs/betterchainc/betterchainc --host $SERVER --port $PORT --wallet-port 8899 set producer inita testera unapprove)"
    # verifyErrorCode "betterchainc unapprove producer"
