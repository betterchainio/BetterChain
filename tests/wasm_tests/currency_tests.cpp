...
BOOST_FIXTURE_TEST_CASE( test_generic_currency, tester ) try {
   produce_blocks(2000);
   create_accounts( {N(currency), N(usera), N(userb)}, asset::from_string("1000.0000 BetterChain") );
   produce_blocks(2);
...
BOOST_FIXTURE_TEST_CASE( test_currency, tester ) try {
   ...
   create_accounts( {N(currency), N(alice), N(bob)}, asset::from_string("1000.0000 BetterChain") );
   transfer( N(inita), N(currency), "10.0000 BetterChain", "memo" );
   produce_block();
