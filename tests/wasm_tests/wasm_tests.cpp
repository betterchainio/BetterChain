...
#include <betterchain/testing/tester.hpp>
...
using namespace betterchain::testing;
using namespace fc;
...
BOOST_FIXTURE_TEST_CASE( basic_test, tester ) try {
   ...
   create_accounts( {N(asserter)}, asset::from_string("1000.0000 BetterChain") );
   transfer( N(inita), N(asserter), "10.0000 BetterChain", "memo" );
   produce_block();
...
BOOST_FIXTURE_TEST_CASE( test_api_bootstrap, tester ) try {
   ...
   create_accounts( {N(tester)}, asset::from_string("1000.0000 BetterChain") );
   transfer( N(inita), N(tester), "10.0000 BetterChain", "memo" );
   produce_block();
...
BOOST_FIXTURE_TEST_CASE( test_proxy, tester ) try {
   ...
   create_account( N(proxy), asset::from_string("10000.0000 BetterChain") );
   create_accounts( {N(alice), N(bob)}, asset::from_string("1000.0000 BetterChain") );
   transfer( N(inita), N(alice), "10.0000 BetterChain", "memo" );
   produce_block();
   ...
   transfer(N(alice), N(proxy), "5.0000 BetterChain");

   while(control->head_block_time() < expected_delivery) {
      control->push_deferred_transactions(true);
      produce_block();
      BOOST_REQUIRE_EQUAL(get_balance( N(alice)), asset::from_string("5.0000 BetterChain").amount);
      BOOST_REQUIRE_EQUAL(get_balance( N(proxy)), asset::from_string("5.0000 BetterChain").amount);
      BOOST_REQUIRE_EQUAL(get_balance( N(bob)),   asset::from_string("0.0000 BetterChain").amount);
   }

   control->push_deferred_transactions(true);
   BOOST_REQUIRE_EQUAL(get_balance( N(alice)), asset::from_string("5.0000 BetterChain").amount);
   BOOST_REQUIRE_EQUAL(get_balance( N(proxy)), asset::from_string("0.0000 BetterChain").amount);
   BOOST_REQUIRE_EQUAL(get_balance( N(bob)),   asset::from_string("5.0000 BetterChain").amount);
...
BOOST_FIXTURE_TEST_CASE( test_deferred_failure, tester ) try {
   ...
   create_accounts( {N(proxy), N(bob)}, asset::from_string("10000.0000 BetterChain") );
   create_account( N(alice), asset::from_string("1000.0000 BetterChain") );
   transfer( N(inita), N(alice), "10.0000 BetterChain", "memo" );
   produce_block();
   ...
   // for now wasm "time" is in seconds, so we have to truncate off any parts of a second that may have applied
   fc::time_point expected_delivery(fc::seconds(control->head_block_time().sec_since_epoch()) + fc::seconds(10));
   auto trace = transfer(N(alice), N(proxy), "5.0000 BetterChain");
   ...
   BOOST_REQUIRE_EQUAL(get_balance( N(alice)), asset::from_string("10.0000 BetterChain").amount);
   BOOST_REQUIRE_EQUAL(get_balance( N(proxy)), asset::from_string("0.0000 BetterChain").amount);
   BOOST_REQUIRE_EQUAL(get_balance( N(bob)),   asset::from_string("0.0000 BetterChain").amount);
...
BOOST_FIXTURE_TEST_CASE( check_entry_behavior, tester ) try {
   ...
   create_accounts( {N(entrycheck)}, asset::from_string("1000.0000 BetterChain") );
   transfer( N(inita), N(entrycheck), "10.0000 BetterChain", "memo" );
   produce_block();
...
BOOST_FIXTURE_TEST_CASE( simple_no_memory_check, tester ) try {
   ...
   create_accounts( {N(nomem)}, asset::from_string("1000.0000 BetterChain") );
   transfer( N(inita), N(nomem), "10.0000 BetterChain", "memo" );
   produce_block();
...
BOOST_FIXTURE_TEST_CASE( check_global_reset, tester ) try {
   ...
   create_accounts( {N(globalreset)}, asset::from_string("1000.0000 BetterChain") );
   transfer( N(inita), N(globalreset), "10.0000 BetterChain", "memo" );
   produce_block();
...
BOOST_FIXTURE_TEST_CASE( memory_operators, tester ) try {
   ...
   create_accounts( {N(current_memory)}, asset::from_string("1000.0000 BetterChain") );
   transfer( N(inita), N(current_memory), "10.0000 BetterChain", "memo" );
   produce_block();
