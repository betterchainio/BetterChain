...
BOOST_AUTO_TEST_CASE( transfer_test ) { try {
  ...
  test.create_accounts( {N(dan), N(bart)}, asset::from_string("100.0000 BetterChain") );
  test.transfer( N(inita), N(dan), "10.0000 BetterChain", "memo" );

  {
     const auto& dans_balance = test.get_balance( N(dan) );
     FC_ASSERT( dans_balance == asset::from_string("10.0000 BetterChain") );
  }
  ...
  /// insufficient funds
  BOOST_REQUIRE_THROW(test.transfer(N(dan),N(bart), "11.0000 BetterChain", "memo"), action_validate_exception);

  /// this should succeed because dan has sufficient balance
  test.transfer(N(dan),N(bart), "10.0000 BetterChain", "memo");


  /// verify that bart now has 10.000
  const auto& barts_balance = test.get_balance( N(bart) );
  FC_ASSERT( barts_balance == asset::from_string("10.0000 BetterChain") );

  {
     /// verify that dan now has 0.000
     const auto& dans_balance = test.get_balance( N(dan) );
     FC_ASSERT( dans_balance == asset::from_string("0.0000 BetterChain") );
  }
...
BOOST_AUTO_TEST_CASE( transfer_delegation ) { try {
   tester test;
   test.produce_blocks(1000);
   test.create_accounts( {N(dan),N(bart),N(trust)}, asset::from_string("100.0000 BetterChain") );
   test.transfer( N(inita), N(dan), "10.0000 BetterChain" );
   test.transfer( N(inita), N(trust), "10.0000 BetterChain" );
   test.transfer( N(trust), N(dan), "1.0000 BetterChain" );
