...
#include <lib/raw_fwd.hpp>

namespace betterchain { namespace raw {
...
namespace betterchain {
   ...
   Type current_message_ex() {
      ...
      char* data = (char *)betterchain::malloc(size);
      assert(data && read_action(data, size) == size, "error reading message");
      Type value;
      betterchain::raw::unpack(data, size, value);
      betterchain::free(data);
      return value;
   }
   ...
   void dump(const key_value2& value, int tab=0) {
      print_ident(tab);print("key:[");print("]\n");
      print_ident(tab);print("value:[");print("\n"); betterchain::dump(value.value, tab+1);print_ident(tab);print("]\n");
   }
   template<>
