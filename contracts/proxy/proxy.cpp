...
#include <lib/native_currency.hpp>
...
namespace proxy {
   ...
   void apply_setowner(set_owner params) {
      ...
      betterchain::print("Setting owner to: ", name(params.owner), " with delay: ", params.delay, "\n");
      configs::store(code_config, self);
   }
   ...
   void apply_onerror( const deferred_transaction& failed_dtrx ) {
      ...
      betterchain::print("Resending Transaction: ", failed_dtrx.sender_id, " as ", id, "\n");
