 ...
 *  "currencya" is called and "exchange" is listed in the notifiers.
 */
void apply_betterchain_transfer( const betterchain::transfer& transfer ) {
   if( transfer.to == N(exchange) ) {
      modify_account( transfer.from, [&]( account& mod_account ){
         mod_account.betterchain_balance += transfer.quantity;
      });
   } else if( transfer.from == N(exchange) ) {
      require_auth( transfer.to ); /// require the receiver of funds (account owner) to authorize this transfer
...
void match( bid& bid_to_match, account& buyer, ask& ask_to_match, account& seller ) {
   ...
   betterchain::tokens ask_betterchain = ask_to_match.quantity * ask_to_match.at_price;

   betterchain_tokens      fill_amount_betterchain = min<betterchain::tokens>( ask_betterchain, bid_to_match.quantity );
   currency_tokens fill_amount_currency;

   if( fill_amount_betterchain == ask_betterchain ) { /// complete fill of ask_to_match
      fill_amount_currency = ask_to_match.quantity;
   } else { /// complete fill of buy
      fill_amount_currency = fill_amount_betterchain / ask_to_match.at_price;
   }
   ...
   bid_to_match.quantity -= fill_amount_betterchain;
   seller.betterchain_balance += fill_amount_betterchain;
...
void apply_exchange_buy( buy_order order ) {
   ...
   assert( exchange_bid.quantity > betterchain::tokens(0), "invalid quantity" );
   ...
   buyer_account.betterchain_balance -= exchange_bid.quantity;
   ...
   if( exchange_bid.quantity ) {
      print( exchange_bid.quantity, " betterchain left over" );
      assert( !order.fill_or_kill, "order not completely filled" );
      bids::store( exchange_bid );
      return;
   }
...
void apply_exchange_sell( sell_order order ) {
   ...
   while( highest_bid.at_price >= exchange_ask.at_price ) {
      ...
      if( highest_bid.quantity == betterchain_tokens(0) ) {
         buyer_account.open_orders--;
         save( seller_account );
         save( buyer_account );
...
void apply_exchange_cancel_buy( order_id order ) {
   ...
   buyer_account.betterchain_balance += bid_to_cancel.quantity;
   buyer_account.open_orders--;
...
//   void precondition( uint64_t code, uint64_t action ) { }
   ...
   void apply( uint64_t code, uint64_t action ) {
      ...
      else if( code == N(betterchain) ) {
        if( action == N(transfer) ) 
           apply_betterchain_transfer( current_action<betterchain::transfer>() );
      } 
      else {
      }
   }
