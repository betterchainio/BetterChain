...
unsigned int test_db::key_str_table() {
    ...
    betterchain::var_table<N(tester), N(tester), N(atr), char*> StringTableAtr;
    betterchain::var_table<N(tester), N(tester), N(ztr), char*> StringTableZtr;
    betterchain::var_table<N(tester), N(tester), N(str), char*> StringTableStr;
...
unsigned int test_db::key_i64i64i64_general() {
  ...
  #define TABLE1_ASSERT(I, V, msg) \
    if(LOGME) {\
      betterchain::print(msg, " : ", res, " a:", V.a, " b:", V.b, " c:", V.c, " t:", V.table, "inx:", uint64_t(I), " ("); \
      betterchain::print(BS(res == sizeof(V)), " ", BS(records[I].a == V.a), " ", BS(records[I].b == V.b), " ", BS(records[I].c == V.c), " => ", N(table2), ")\n"); \
    } \
  ...
  #define LOAD_OK(I, O, T, INX, MSG) \
    {betterchain::remove_reference<decltype(V)>::type tmp; my_memset(&tmp, 0, sizeof(tmp));tmp = V; \
    res = LOAD(I, O, T, tmp); \
  ...
  { \
    ...
    auto j = 0; \
    do { \
      betterchain::remove_reference<decltype(records[0])>::type tmp = records[I[j]]; \
  ...
  { \
    ...
    auto j = 0; \
    do { \
      betterchain::remove_reference<decltype(records[0])>::type tmp = records[j]; \
...
unsigned int test_db::key_str_setup_limit()
{
   ...
   char* value = static_cast<char*>(betterchain::malloc(value_size));
   value[4] = '\0';
...
unsigned int test_db::key_str_another_under_limit()
{
   ...
   set_key_str(2, key);
   value = static_cast<char*>(betterchain::realloc(value, value_size));
   update_str(N(dblimits), N(dblstr), key, sizeof(key), value, value_size);
   betterchain::free(value);
...
unsigned int test_db::key_i64_setup_limit()
{
   ...
   auto value = (uint64_t*)betterchain::malloc(value_size);
   for(int i = 0; i < 1024 * 2; ++i)
   {
      value[0] = i;
...
unsigned int test_db::key_i64_another_under_limit()
{
   ...
   value = (uint64_t*)betterchain::realloc(value, value_size);
   value[0] = 1024 * 2;
   store_i64(N(dblimits), N(dbli64), (char*)value, value_size);
...
unsigned int test_db::key_i128i128_setup_limit()
{
   ...
   // 1024 * 2 * (2528 + 32) = 5,242,880
   const uint64_t value_size = 315 * sizeof(uint64_t) + 1;
   auto value = (uint128_t*)betterchain::malloc(value_size);
   for(int i = 0; i < 1024 * 2; ++i)
   {
      value[0] = i;
      value[1] = value[0] + 1;
...
unsigned int test_db::key_i128i128_another_under_limit()
{
   ...
   value = (uint128_t*)betterchain::realloc(value, value_size);
   value[0] = 1024 * 2;
   value[1] = value[0] + 1;
   store_i128i128(N(dblimits), N(dbli128i128), (char*)value, value_size);
