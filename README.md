...
3. [Building BetterChain and running a node](#runanode)
...
8. [Running BetterChain in Docker](#docker)
...
For Ubuntu 16.10 and MacOS Sierra, there is an automated build script that can install all dependencies and builds BetterChain.

It is called build.sh with the following inputs.
- architecture [ubuntu|darwin]
- optional mode [full|build]

The second optional input can be `full` or `build` where `full` implies that it installs dependencies and builds betterchain. If you omit this input then the build script installs dependencies and then builds betterchain.

```bash
./build.sh <architecture> <optional mode>
```
Choose whether you will be building for a local testnet or for the public testnet and jump to the appropriate section below.  Clone the BetterChain repository recursively as described and run build.sh located in the root `betterchain` folder.
...
<a name="runanode"></a>
## Building BetterChain and running a node
...
This is needed to compile the example contracts inside `betterchain/contracts` folder and their respective tests.
...
After successfully building the project, the `betterchaind` binary should be present in the `build/programs/betterchaind` directory. Run `betterchaind` -- it will probably exit with an error, but if not, close it immediately with <kbd>Ctrl-C</kbd>. If it exited with an error, note that `betterchaind` created a directory named `data-dir` containing the default configuration (`config.ini`) and some other internals. This default data storage path can be overridden by passing `--data-dir /path/to/data` to `betterchaind`.  These instructions will continue to use the default directory.
...
plugin = betterchain::producer_plugin
# Wallet plugin
plugin = betterchain::wallet_api_plugin
# As well as API and HTTP plugins
plugin = betterchain::chain_api_plugin
plugin = betterchain::http_plugin
```

Now it should be possible to run `betterchaind` and see it begin producing blocks.
...
BetterChain comes with example contracts that can be uploaded and run for testing purposes. Next we demonstrate how to upload and interact with the sample contract "currency".
...
```bash
./betterchainc wallet import 5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3
```
...
Public key: BetterChainXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
```
...
```bash
./betterchainc create account inita currency PUBLIC_KEY_1 PUBLIC_KEY_2
```
...
```bash
./betterchainc get account currency
```
...
```json
{
  ...
  "betterchain_balance": "0.0000 BetterChain",
  "staked_balance": "0.0001 BetterChain",
  "unstaking_balance": "0.0000 BetterChain",
...
```bash
./betterchainc wallet import XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
```
...
```bash
./betterchainc get code currency
...
```bash
./betterchainc set contract currency ../../contracts/currency/currency.wast ../../contracts/currency/currency.abi
```
...
```bash
./betterchainc push action currency issue '{"to":"currency","quantity":"1000.0000 CUR"}' --permission currency@active
```

Next verify the currency contract has the proper initial balance:

```bash
./betterchainc get table currency currency account
{
  "rows": [{
     "currency": 1381319428,
     "balance": 10000000
...
```bash
./betterchainc push action currency transfer '{"from":"currency","to":"inita","quantity":"20.0000 CUR","memo":"my first transfer"}' --permission currency@active
```
...
```bash
./betterchainc push action currency transfer '{"from":"${usera}","to":"${userb}","quantity":"20.0000 CUR","memo":""}' --permission ${usera}@active
```
...
```bash
./betterchainc get table inita currency account
{
  "rows": [{
      "currency": 1381319428,
      "balance": 200000
...
To run a local testnet you can use a `launcher` application provided in the `~/betterchain/build/programs/launcher` folder.
...
```bash
spawning child, programs/betterchaind/betterchaind --skip-transaction-signatures --data-dir tn_data_0
spawning child, programs/betterchaind/betterchaind --skip-transaction-signatures --data-dir tn_data_1
```

To confirm the nodes are running, run the following `betterchainc` commands:
```bash
~/betterchain/build/programs/betterchainc
./betterchainc -p 8888 get info
./betterchainc -p 8889 get info
```
...
```bash
Launched betterchaind.
See testnet_np/stderr.txt for betterchaind output.
Synching requires at least 8 minutes, depending on network conditions.
```

To confirm betterchaind operation and synchronization:
...
For the `master` branch: https://betterchain.github.io/betterchain/
For the public testnet branch: http://htmlpreview.github.io/?https://github.com/betterchainio/betterchain/blob/dawn-2.x/docs/index.html
...
To use the WASM compiler, BetterChain has an external dependency on [binaryen](https://github.com/WebAssembly/binaryen.git):
...
Your environment is set up. Now you can <a href="#runanode">build BetterChain and run a node</a>.
